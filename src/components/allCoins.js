...
const AllCoinsData = () => {
  ...
  const getCoins = useSelector((state) => state.coinReducer);
  ...
  return (
    <div>
      ...
      <Link className={styles.link} to="/details/allcoins" onClick={() => dispatch(displayCoins(allCoin.name))}>
      ...
      <div className={styles.subMain}>
        <ul className={styles.list}>
          {categories.map((coin) => (
            <Link key={coin.name} className={styles.link} to={`/details/${coin.name}`} onClick={() => dispatch(displayCoins(coin.name))}>
